...
struct Button{
	...
	ivec2 pos;
	ivec2 size;
	uchar state=BUTTON_VISIBLE|BUTTON_CAN_HOVER;	//Bits: Sichtbarkeit, Maus hover aktiv, Maus hover state, Button gedr�ckt, Rest ungenutzt
...
inline void buttonsClicked(Button* buttons, uint button_count, Mouse& mouse){
	for(uint i=0; i < button_count; ++i){
		...
		if(delta.x >= 0 && delta.x <= b.size.x && delta.y >= 0 && delta.y <= b.size.y){
			...
			if(LMB(mouse) && !checkButtonState(b, BUTTON_PRESSED)){
				ErrCheck(b.event());
				b.state |= BUTTON_PRESSED;
			}
			else if(!LMB(mouse)) b.state &= ~BUTTON_PRESSED;
...
struct Menu{
	Button* buttons;
	uint button_count=0;
	uchar state=0;	//Bits: offen, toggle bit f�r offen, Rest ungenutzt
};
enum MENUSTATE{
	MENU_OPEN=1,
	MENU_OPEN_TOGGLE=2
